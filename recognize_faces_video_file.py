...
# initialize pointer to video file and video writer
print("[INFO] processing video...")
stream = cv2.VideoCapture(args["input"])
writer = None

# variables for frame capturing
frame_count = 0
capture_interval = 24  # Capture 1 frame per second (24 frames per second)
...
while True:
    ...
    # detect (x, y)-coordinates of bounding boxes corresponding to each face in input frame, then compute facial embeddings for each face
    ...
    for encoding in encodings:
        ...
        # check if match
        if True in matches:
            # find indexes of all matched faces, initialize dict to count total num times face matched
            matchedIdxs = [i for (i, b) in enumerate(matches) if b]
            counts = {}
